...
class App(models.Model):
    ...
    icon = models.ImageField(
        upload_to='app_icon_thumbnail/%Y-%m-%d/',
        blank=True)
...
class Comment(models.Model):
    ...
    def __str__(self):
        return 'Comment on %s by %s' % (
            self.app.title, self.user.get_full_name)
...
class Download(models.Model):
    ...
    download_option = models.CharField(
        max_length=1, default='I', choices=CHOICES)
...
def update_download_link(sender, instance=None, created=False, **kwargs):
    if created:
        ...
        link = "http://ns-apps.ee.washington.edu/" + \
            instance.app.name + "/#cy-app-installation-tab"
        if choice == 'I':
...
def update_thumbnail(sender, instance=None, created=False, **kwargs):
    if created:
        instance.thumbnail = scale_img(
            instance.screenshot, instance.screenshot.name, 150, 'h')
        instance.save()
