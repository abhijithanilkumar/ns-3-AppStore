...
class AppTestCase(TestCase):
    ...
    def test_app_created(self):
        ...
        self.assertEqual(
            app.mailing_list,
            'http://mailman-testapp.com/test-app-developer/')
...
class DevelopmentTestCase(TestCase):
    def setUp(self):
        ...
        Development.objects.create(
            ...
            filename=SimpleUploadedFile(
                'development.md',
                b'This is a sample file for Development'))
    ...
    def test_development_created(self):
        ...
        self.assertEqual(
            development.app.abstract,
            'This is a test App for Development')
        self.assertEqual(
...
class DownloadTestCase(TestCase):
    def setUp(self):
        downloadApp = App.objects.create(
            name='DoAp',
            title='App Title',
            app_type='F',
            abstract='This is a test App for Download',
            description='This is a test App for Download')
        release_nsrelease = NsRelease.objects.create(
            name='NsR1', url='https://www.nsnam.org/ns-3.29/')
    ...
    def test_download_created(self):
        ...
        self.assertEqual(
            download.app.abstract,
            'This is a test App for Download')
        self.assertEqual(
        ...
        self.assertEqual(
            download.default_release.app.title,
            download.app.title)
        self.assertEqual(
            download.default_release.app.app_type,
            download.app.app_type)
        self.assertEqual(
            download.default_release.app.abstract,
            download.app.abstract)
        self.assertEqual(
            download.default_release.app.description,
            download.app.description)
        self.assertEqual(download.default_release.version, '0.1')
        self.assertEqual(download.default_release.require.name, 'NsR1')
        self.assertEqual(
            download.default_release.require.url,
            'https://www.nsnam.org/ns-3.29/')
        self.assertEqual(
            download.default_release.date,
            datetime.date(
                2018,
                11,
                1))
        self.assertEqual(
            download.default_release.notes,
            '## usage \n This is a test Markdown text')
        f = open(download.default_release.filename.url[1:])
        self.assertEqual(f.read(), 'Test for Download')
        f.close()
        self.assertEqual(
            download.default_release.url,
            'https://www.nsnam.org/ns-3.29/')
        self.assertEqual(
            download.external_url,
            'http://test-app.myapp.com/download')


class InstallationTestCase(TestCase):
    def setUp(self):
        installation_app = App.objects.create(
            name='Ins1',
            title='App Title',
            app_type='F',
    ...
    def test_installation_created(self):
        ...
        self.assertEqual(
            installation.app.abstract,
            'This is a test App for Installation')
        self.assertEqual(
...
class MaintenanceTestCase(TestCase):
    def setUp(self):
        maintenance_app = App.objects.create(
            name='Mnt1',
            title='App Title',
            app_type='F',
    ...
    def test_maintenance_created(self):
        ...
        self.assertEqual(
            maintenance.app.abstract,
            'This is a test App for Maintenance')
        self.assertEqual(
            maintenance.app.description,
            'This is a test App for Maintenance')
        self.assertEqual(
            maintenance.notes,
            '#MAINTENANCE \n View parent repository for maintenance details')
